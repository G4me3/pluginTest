...
var AnnotationCreation = /*#__PURE__*/function (_Component) {
  ...
  function AnnotationCreation(props) {
    ...
    _this.state = _extends({}, toolState, {
      annoBody: '',
  ...
  _proto.submitForm = function submitForm(e) {
    ...
    var _this$state = this.state,
      ...
      svg = _this$state.svg,
      textEditorStateBustingKey = _this$state.textEditorStateBustingKey;
    canvases.forEach(function (canvas) {
      ...
      if (annotation) {
        storageAdapter.update(anno).then(function (annoPage) {
          receiveAnnotation(canvas.id, storageAdapter.annotationPageId, annoPage);
        });
      } else {
        storageAdapter.create(anno).then(function (annoPage) {
          receiveAnnotation(canvas.id, storageAdapter.annotationPageId, annoPage);
        });
      }
    });
  ...
  _proto.render = function render() {
    ...
    var _this$state2 = this.state,
      ...
      textEditorStateBustingKey = _this$state2.textEditorStateBustingKey;
    ...
    activeTool === 'freehand' ? /*#__PURE__*/_react["default"].createElement(_ToggleButtonGroup["default"], {
      ...
      autoFocus: true,
      role: "listbox"
    }, [1, 3, 5, 10, 50].map(function (option, index) {
...
AnnotationCreation.propTypes = process.env.NODE_ENV !== "production" ? {
  ...
  config: _propTypes["default"].shape({
    annotation: _propTypes["default"].shape({
      ...
      defaults: _propTypes["default"].objectOf(_propTypes["default"].oneOfType([_propTypes["default"].bool, _propTypes["default"].func, _propTypes["default"].number, _propTypes["default"].string]))
    })
  }).isRequired,
